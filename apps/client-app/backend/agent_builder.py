...
- NEVER select a raw *_id / id column as something a person will read. A human asking "who works on this project" wants names, not UUIDs. If the schema has a human-readable label for the same row — a sibling column on the same table (assignee_id + assignee_name on the same tickets row), or a name/title column on the table an ID points to (join to it) — select that instead of the ID. Only include an ID column in the SELECT list if there is truly no name/label anywhere for that entity in the whole schema.
...
def _test_run_tool(name: str, code: str, has_required_filter: bool, warnings: list[str]) -> None:
    ...
    if result.startswith("No matching data found") or result.startswith("Query failed"):
        ...
        return
    # A raw UUID in the output is a strong, schema-independent signal that
    # this selected an *_id column instead of the human-readable name for
    # the same row — real bug found live: "who works on this project"
    # returned a list of UUIDs instead of employee names. Column-name
    # heuristics in the prompt can miss this; the actual returned values
    # can't lie about what they look like.
    if re.search(r"\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b", result, re.IGNORECASE):
        warnings.append(f"Tool '{name}' returned raw ID values (e.g. UUIDs) instead of readable names — review it (the 'View / edit generated code' button) and pick a name/label column instead before relying on it.")
